...
class InteractiveCube(plt.Axes):
    ...
    def _initialize_widgets(self):
        ...
        ideal_button_spacing = (1 - 2 * edge_spacing - num_buttons * button_width) / (num_buttons - 1)

        self._ax_reset = self.figure.add_axes([edge_spacing, 0.05, button_width, button_height])
        self._btn_reset = widgets.Button(self._ax_reset, 'Reset View', color="1")
        self._btn_reset.on_clicked(self._reset_view)

        self._ax_random = self.figure.add_axes(
            [edge_spacing + button_width + ideal_button_spacing, 0.05, button_width, button_height])
        self._btn_random = widgets.Button(self._ax_random, 'Randomize', color="1")
        self._btn_random.on_clicked(self._randomize)

        self._ax_solve = self.figure.add_axes(
            [edge_spacing + 2 * (button_width + ideal_button_spacing), 0.05, button_width, button_height])
        self._btn_solve = widgets.Button(self._ax_solve, 'Solve Cube', color="1")
        self._btn_solve.on_clicked(self._solve_cube)

        self._ax_print = self.figure.add_axes(
            [edge_spacing + 3 * (button_width + ideal_button_spacing), 0.05, button_width, button_height])
        self._btn_print = widgets.Button(self._ax_print, 'Print Solution', color="1")
        self._btn_print.on_clicked(self._print_solution)
        ...
        ideal_button_spacing2 = (1 - 2 * edge_spacing2 - num_buttons2 * button_width2) / (num_buttons2 - 1)

        self._ax_F = self.figure.add_axes([edge_spacing2, 0.15, button_width2, button_height2])
        self._btn_F = widgets.Button(self._ax_F, "F", color="1")
        self._btn_F.on_clicked(lambda event: self._key_press(FakeKeyEvent('F', shift=False)))

        self._ax_Fprime = self.figure.add_axes(
            [edge_spacing2 + button_width2 + ideal_button_spacing2, 0.15, button_width2, button_height2])
        self._btn_Fprime = widgets.Button(self._ax_Fprime, "F'", color="1")
        self._btn_Fprime.on_clicked(lambda event, key='F': self._key_press(FakeKeyEvent(key, shift=True)))

        self._ax_R = self.figure.add_axes(
            [edge_spacing2 + 2 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_R = widgets.Button(self._ax_R, "R", color="1")
        self._btn_R.on_clicked(lambda event: self._key_press(FakeKeyEvent('R', shift=False)))

        self._ax_Rprime = self.figure.add_axes(
            [edge_spacing2 + 3 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_Rprime = widgets.Button(self._ax_Rprime, "R'", color="1")
        self._btn_Rprime.on_clicked(lambda event, key='R': self._key_press(FakeKeyEvent(key, shift=True)))

        self._ax_B = self.figure.add_axes(
            [edge_spacing2 + 4 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_B = widgets.Button(self._ax_B, "B", color="1")
        self._btn_B.on_clicked(lambda event: self._key_press(FakeKeyEvent('B', shift=False)))

        self._ax_Bprime = self.figure.add_axes(
            [edge_spacing2 + 5 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_Bprime = widgets.Button(self._ax_Bprime, "B'", color="1")
        self._btn_Bprime.on_clicked(lambda event, key='B': self._key_press(FakeKeyEvent(key, shift=True)))

        self._ax_L = self.figure.add_axes(
            [edge_spacing2 + 6 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_L = widgets.Button(self._ax_L, "L", color="1")
        self._btn_L.on_clicked(lambda event: self._key_press(FakeKeyEvent('L', shift=False)))

        self._ax_Lprime = self.figure.add_axes(
            [edge_spacing2 + 7 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_Lprime = widgets.Button(self._ax_Lprime, "L'", color="1")
        self._btn_Lprime.on_clicked(lambda event, key='L': self._key_press(FakeKeyEvent(key, shift=True)))

        self._ax_U = self.figure.add_axes(
            [edge_spacing2 + 8 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_U = widgets.Button(self._ax_U, "U", color="1")
        self._btn_U.on_clicked(lambda event: self._key_press(FakeKeyEvent('U', shift=False)))

        self._ax_Uprime = self.figure.add_axes(
            [edge_spacing2 + 9 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_Uprime = widgets.Button(self._ax_Uprime, "U'", color="1")
        self._btn_Uprime.on_clicked(lambda event, key='U': self._key_press(FakeKeyEvent(key, shift=True)))

        self._ax_D = self.figure.add_axes(
            [edge_spacing2 + 10 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_D = widgets.Button(self._ax_D, "D", color="1")
        self._btn_D.on_clicked(lambda event: self._key_press(FakeKeyEvent('D', shift=False)))

        self._ax_Dprime = self.figure.add_axes(
            [edge_spacing2 + 11 * (button_width2 + ideal_button_spacing2), 0.15, button_width2, button_height2])
        self._btn_Dprime = widgets.Button(self._ax_Dprime, "D'", color="1")
        self._btn_Dprime.on_clicked(lambda event, key='D': self._key_press(FakeKeyEvent(key, shift=True)))

        # Draw Titles
        self.figure.text(0.5, 0.95,
                         "RUBIKS CUBE SOLVER",
                         size=15, color='w',
                         fontname='fantasy',
                         ha='center')

        self.figure.text(0.5, 0.90,
                         "Created by Justin Hartfield",
                         size=10, color='w',
                         fontname='fantasy',
                         ha='center')
    ...
    def _solve_cube(self, *args):
        ...
        for (face, n, layer) in move_list[::-1]:
            ...
            print(x, face, n, layer)
            x += 1
            # self.figure.canvas.draw()
            t.sleep(2)
        self.cube._move_list = []

    def _randomize(self, *args):
        print("Randomizing ...")
        moves = ['F', "F'", 'R', "R'", 'B', "B'", 'L', "L'", 'U', "U'", 'D', "D'"]

        np.random.shuffle(moves)

        for move in moves:
            self.rotate_face(move[0], -1 if len(move) == 1 else 1)

        self._draw_cube()
    def _print_solution(self, *args):
        global txt
        try:
            txt.remove()
        except:
        ...
        for (face, n, layer) in move_list[::-1]:
            # self.rotate_face(face, -n, layer, steps=3)
            print(str(x) + ".)", face, round(n))
            x += 1
            if round(n) == 1:
                ans.append(face + '_prime')
            elif round(n) == 2:
                ans.append(face + '_prime' + face + '_prime')
            elif round(n) == -1:
                ans.append(face)
            elif round(n) == -2:
                ans.append(face + face)
        print("Solution: ", ans)
        txt = self.figure.text(0.05, 0.01,
                               "Solution: " + str(ans),
                               size=10, color='white')
        self.figure.canvas.draw()
    ...
    def _key_press(self, event):
        if event.key == 'shift':
            ...
            if self._shift or (hasattr(event, 'shift') and event.shift):
                direction = -1
            else:
                direction = 1
